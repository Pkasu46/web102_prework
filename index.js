...
function addGamesToPage(games) {
    ...
    games.forEach(game => {
        ...
        newDiv.innerHTML = `Game Name is: ${game.name}. Game description is: ${game.description}. Game goal is: ${game.goal}. Game backers is: ${game.backers} Game pledged: ${game.pledged}
...
let numOfUnfundedGames = unfundedGames.reduce((acc, game) => {
    return acc + 1;
}, 0);

let totalNumOfGames = GAMES_JSON.reduce((acc, game) => {
    return acc + 1;
}, 0);

// create a string that explains the number of unfunded games using the ternary operator
let unfundedDisplayString = numOfUnfundedGames > 0 ? `A total of ${totalRaised.toLocaleString('en-US')} has been raised for ${totalNumOfGames} games. Currently, ${numOfUnfundedGames} games remain unfunded. We need your help to fund these amazing games!` : 'Currently there are no unfunded games' ;

// create a new DOM element containing the template string and append it to the description container
let unfundedDescription = document.createElement('p');
unfundedDescription.innerHTML = unfundedDisplayString;
